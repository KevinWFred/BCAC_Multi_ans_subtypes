...
compute_pop_score_pvalues=function(pop="euro",scoreres1=euro_onco,scoreres2=euro_icogs,i1=1)
{
  ...
  save(scoreres,file=paste0(scoreoutfolder,"res_",i1,".RData"))
  print(Sys.time())
}
                                       ...
                                       allsigma_onco=euro_allsigma_onco)
{
  ...
  save(intrinsicres,file=paste0(intrinsicoutfolder,"res_",i1,".RData"))
}
...
{
  swarmjobs=data.frame(code=rep("/data/BB_Bioinformatics/Kevin/BCAC/code/compute_metapvalues_population.R",1000),
                       i1=1:1000)
  write.table(swarmjobs,file=paste0("compute_metapvalues_population.swam"),row.names=F,col.names=F,sep="\t",quote=F)
}
#swarm -f /data/BB_Bioinformatics/Kevin/BCAC/code/compute_metapvalues_population.swam -g 48 --module R/4.3 --partition=quick --time=4:00:00 --gres=lscratch:48 -p 2
...
collect_pvalues=function(pop="euro")
{
  ...
  {
    if (i %%200 ==0) cat(i,'..')
    load(paste0(scoreoutfolder,"res_",i,".RData"))
    allscoreres=rbind(allscoreres,scoreres)
    load(paste0(intrinsicoutfolder,"res_",i,".RData"))
...
#save(collect_pvalues_african,file="../result/compute_metapvalues_population.RData")
save(collect_pvalues_euro,collect_pvalues_asian,collect_pvalues_african,file="../result/compute_metapvalues_population.RData")
