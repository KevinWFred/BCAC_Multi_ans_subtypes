...
compute_metap=function(pop="euro",idxstart=1,idxend=10)
{
  ...
  allres=list(metares=metares,metaallsigma=metaallsigma)
  outfolder=paste0("../result/metaresult/",pop)
  if(!dir.exists(outfolder)) dir.create(outfolder)
  save(allres,file=paste0(outfolder,"/res_",i1,".RData"))
}
...
#tmp=sum( sapply(ls(),function(x){object.size(get(x))}))
#sort( sapply(ls(),function(x){object.size(get(x))})) /1E6
#createswarm()
#cd swarm
#swarm -f /data/BB_Bioinformatics/Kevin/BCAC/code/compute_metap_euro.swarm -g 20 --module R/4.3 --partition=quick --time=00:4:00 --gres=lscratch:20 -p 2
